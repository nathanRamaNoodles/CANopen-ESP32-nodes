...
CO_ReturnError_t CO_GTWA_init(CO_GTWA_t *gtwa,
  ...
  CO_fifo_init(&gtwa->commFifo, &gtwa->commBuf[0],
               CO_CONFIG_GTWA_COMM_BUF_SIZE + 1);

#if (CO_CONFIG_GTW) & CO_CONFIG_GTW_ASCII_LOG
  CO_fifo_init(&gtwa->logFifo, &gtwa->logBuf[0],
...
void CO_GTWA_initRead(CO_GTWA_t *gtwa,
                      size_t (*readCallback)(void *object, const char *buf,
                                             size_t count),
...
static const char CO_GTWA_helpString[] =
    ...
    "[<net>] set sdo_block <value>            # Enable/disable SDO block "
    "transfer.\n"
    "\n"
...
static inline uint32_t getU32(char *token, uint32_t min, uint32_t max,
                              bool_t *err) {
  char *sRet;
...
static bool_t checkNetNode(CO_GTWA_t *gtwa, int32_t net, int16_t node,
                           uint8_t NodeMin, CO_GTWA_respErrorCode_t *errCode) {
  bool_t e = false;
...
static const CO_GTWA_dataType_t dataTypes[] = {
    ...
    //    {"os",  0, CO_GWA_dtpHex, CO_DWA_dtsHex}, /* ochar_t (OCTET_STRING)
    //    (mime-base64 (RFC2045) should be used here) */
    //    {"us",  0, CO_GWA_dtpHex, CO_DWA_dtsHex}, /*  (UNICODE_STRING)
    //    (mime-base64 (RFC2045) should be used here) */
    //    {"d",   0, CO_GWA_dtpHex, CO_DWA_dtsHex}  /* domain_t (DOMAIN)
    //    (mime-base64 (RFC2045) should be used here) */
};
 ...
 * PROCESS FUNCTION
 ******************************************************************************/
void CO_GTWA_process(CO_GTWA_t *gtwa, bool_t enable, uint32_t timeDifference_us,
  ...
  while (gtwa->state == CO_GTWA_ST_IDLE &&
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (err || n == 0) {
        ...
        err = true;
        break;
      } else if (isdigit((unsigned char)tok[0]) == 0) {
        /* <command> found */
        break;
      } else if (closed != 0) {
    ...
    /* Download SDO comm. - w[rite] <index> <subindex> <datatype> <value> */
      ...
      SDO_ret = CO_SDOclientDownloadInitiate(
          gtwa->SDO_C, idx, subidx, gtwa->SDOdataType->length,
    ...
    else if (strcmp(tok, "preop") == 0 || strcmp(tok, "preoperational") == 0) {
      CO_ReturnError_t ret;
      bool_t NodeErr = checkNetNode(gtwa, net, node, 0, &respErrorCode);
  ...
  /* SDO upload state */
    ...
    ret = CO_SDOclientUpload(gtwa->SDO_C, timeDifference_us, &abortCode, NULL,
                             &sizeTransferred, timerNext_us);
    ...
    /* Response data must be read, partially or whole */
      ...
      if (!gtwa->SDOdataCopyStatus) {
        gtwa->respBufCount = snprintf(gtwa->respBuf, CO_GTWA_RESP_BUF_SIZE - 2,
                                      "[%" PRId32 "] ", gtwa->sequence);
  ...
  /* SDO download state */
    ...
    if (gtwa->SDOdataCopyStatus) {
      CO_fifo_st status;
      gtwa->SDOdataType->dataTypeScan(&gtwa->SDO_C->bufFifo, &gtwa->commFifo,
                                      &status);
      /* set to true, if command delimiter was found */
      closed = ((status & CO_fifo_st_closed) == 0) ? 0 : 1;
      /* set to true, if data are copied only partially */
     ...
     * timeout */
    if (gtwa->SDOdataCopyStatus && CO_fifo_getOccupied(&gtwa->SDO_C->bufFifo) <
    ...
    if (!hold) {
      ...
      do {
        ret = CO_SDOclientDownload(gtwa->SDO_C, timeDifference_us, abort,
    ...
    CO_LSSmaster_return_t ret;
    ret = CO_LSSmaster_switchStateSelect(gtwa->LSSmaster, timeDifference_us,
                                         NULL);
    ...
    CO_LSSmaster_return_t ret;
    ret = CO_LSSmaster_configureNodeId(gtwa->LSSmaster, timeDifference_us,
                                       gtwa->lssNID);
    if (ret != CO_LSSmaster_WAIT_SLAVE) {
    ...
    CO_LSSmaster_return_t ret;
    ret = CO_LSSmaster_configureBitTiming(gtwa->LSSmaster, timeDifference_us,
                                          gtwa->lssBitrate);
    if (ret != CO_LSSmaster_WAIT_SLAVE) {
      if (ret == CO_LSSmaster_OK_ILLEGAL_ARGUMENT) {
      ...
      if (ret == CO_LSSmaster_OK) {
        gtwa->respBufCount =
            snprintf(gtwa->respBuf, CO_GTWA_RESP_BUF_SIZE,
                     ...
                     gtwa->sequence, gtwa->lssAddress.identity.vendorID,
      ...
      if (ret == CO_LSSmaster_OK || ret == CO_LSSmaster_SCAN_FINISHED) {
        gtwa->respBufCount =
            snprintf(gtwa->respBuf, CO_GTWA_RESP_BUF_SIZE,
                     "[%" PRId32 "] 0x%08" PRIX32 " 0x%08" PRIX32
                     " 0x%08" PRIX32 " 0x%08" PRIX32 "\r\n",
                     gtwa->sequence, gtwa->lssFastscan.found.identity.vendorID,
      ...
      if (ret != CO_LSSmaster_WAIT_SLAVE) {
        ...
        if (ret == CO_LSSmaster_OK || ret == CO_LSSmaster_SCAN_NOACK) {
          ...
          gtwa->respBufCount = snprintf(gtwa->respBuf, CO_GTWA_RESP_BUF_SIZE,
                                        ...
                                        "[%" PRId32 "] OK\r\n",
                                        gtwa->lssNodeCount, gtwa->sequence);
          respBufTransfer(gtwa);
          gtwa->state = CO_GTWA_ST_IDLE;
  ...
  /* Print help string (in multiple segments if necessary) */
    ...
    do {
      ...
      memcpy(gtwa->respBuf, &gtwa->helpString[gtwa->helpStringOffset],
             lenCopied);
  ...
  /* print CANopen status LED diodes */
    ...
    if (i != gtwa->ledStringPreviousIndex) {
      gtwa->respBufCount = snprintf(gtwa->respBuf, CO_GTWA_RESP_BUF_SIZE, "%s",
