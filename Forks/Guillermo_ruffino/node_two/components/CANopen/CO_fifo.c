 ...
 ******************************************************************************/
size_t CO_fifo_write(CO_fifo_t *fifo, const char *buf, size_t count,
                     uint16_t *crc) {
  size_t i;
  char *bufDest;
...
bool_t CO_fifo_CommSearch(CO_fifo_t *fifo, bool_t clear) {
  ...
  if (commandEnd != NULL) {
    ...
    commandEnd = (char *)memchr((const void *)&fifo->buf[0], DELIM_COMMAND,
                                fifo->writePtr);
...
bool_t CO_fifo_trimSpaces(CO_fifo_t *fifo) {
  ...
  if (fifo != NULL) {
    while (fifo->readPtr != fifo->writePtr) {
      unsigned char c = fifo->buf[fifo->readPtr];
...
/******************************************************************************/
size_t CO_fifo_readToken(CO_fifo_t *fifo, char *buf, size_t count, char *closed,
                         bool_t *err) {
  bool_t delimCommandFound = false;
  ...
  while (destSpace > 0 && (st & CO_fifo_st_errMask) == 0 && !finished) {
    ...
    if (dest->started == false) {
      ...
      if (isxdigit((unsigned char)c) != 0) {
        ...
        finished = true;
      } else if (isgraph((unsigned char)c) != 0) {
      ...
      if (isxdigit((unsigned char)c) != 0) {
        ...
        destSpace--;
      } else if (isgraph((unsigned char)c) != 0 && c != DELIM_COMMENT) {
        /* printable character, not hex digit, error */
        st |= CO_fifo_st_errTok;
      } else {
...
size_t CO_fifo_cpyTok2Vs(CO_fifo_t *dest, CO_fifo_t *src, CO_fifo_st *status) {
  ...
  while (destSpace > 0 && (st & CO_fifo_st_errMask) == 0 && !finished) {
    ...
    case 0: /* beginning of the string, first write into dest */
      if (isgraph((unsigned char)c) == 0 || c == DELIM_COMMENT) {
    ...
    case 2: /* inside string, single word, no quotes */
      if (c == '"') {
        ...
        dest->aux += 2;
      } else if (isgraph((unsigned char)c) == 0 && dest->aux == 2) {
