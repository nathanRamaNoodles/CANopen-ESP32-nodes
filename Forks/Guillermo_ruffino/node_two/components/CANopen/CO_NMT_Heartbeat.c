...
static void CO_NMT_receive(void *object, void *msg)
{
    ...
    ESP_LOGI("CO", "CO_NMT_receive DLC: %d", DLC);
    ...
    if (errorBehavior && (NMT->operatingState == CO_NMT_OPERATIONAL))
    {
        ...
        if (errorRegister)
        {
            ...
            if (errorRegister & CO_ERR_REG_COMM_ERR)
            {
                ...
                else if (CO_isError(NMT->emPr->em, CO_EM_CAN_TX_BUS_OFF) || CO_isError(NMT->emPr->em, CO_EM_HEARTBEAT_CONSUMER) || CO_isError(NMT->emPr->em, CO_EM_HB_CONSUMER_REMOTE_RESET))
                {
