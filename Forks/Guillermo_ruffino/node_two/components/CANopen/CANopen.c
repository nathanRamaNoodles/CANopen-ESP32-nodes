...
#if CO_NO_SYNC > 1 || CO_NO_EMERGENCY != 1 || (CO_NO_SDO_SERVER < 1 || CO_NO_SDO_SERVER > 128) || CO_NO_TIME > 1 || CO_NO_SDO_CLIENT > 128 || (CO_NO_RPDO < 1 || CO_NO_RPDO > 0x200) || (CO_NO_TPDO < 1 || CO_NO_TPDO > 0x200) || ODL_consumerHeartbeatTime_arrayLength == 0 || ODL_errorStatusBits_stringLength < 10 || CO_NO_LSS_SLAVE > 1 || CO_NO_LSS_MASTER > 1
                              ...
                              uint32_t *timerNext_us)
{
    ...
    CO_LEDs_process(co->LEDs,
                    ...
                    unc ? false : (CO_isError(co->em, CO_EM_HEARTBEAT_CONSUMER) || CO_isError(co->em, CO_EM_HB_CONSUMER_REMOTE_RESET)),
                    OD_errorRegister != 0,
