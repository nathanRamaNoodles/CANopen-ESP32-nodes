...
static void CO_SDOclient_receive(void *object, void *msg)
{
    ...
    ESP_LOGI("CO", "CO_SDOclient_receive DLC: %d", DLC);
    ...
    if (SDO_C->state != CO_SDO_ST_IDLE && DLC == 8U && !CO_FLAG_READ(SDO_C->CANrxNew))
    {
#if (CO_CONFIG_SDO_CLI) & CO_CONFIG_SDO_CLI_BLOCK
...
{
    /* verify arguments */
    if (SDO_C == NULL || SDOClientPar == NULL || SDOClientPar->maxSubIndex != 3 || CANdevRx == NULL || CANdevTx == NULL)
    {
                                                   ...
                                                   bool_t blockEnable)
{
    ...
    if (SDO_C->SDO != NULL && SDO_C->SDOClientPar->nodeIDOfTheSDOServer == SDO_C->SDO->nodeId)
    {
                                      ...
                                      size_t sizeIndicated)
{
    if (SDO_C != NULL)
    {
        SDO_C->sizeInd = sizeIndicated;
#if (CO_CONFIG_SDO_CLI) & CO_CONFIG_SDO_CLI_BLOCK
        if (SDO_C->state == CO_SDO_ST_DOWNLOAD_BLK_INITIATE_REQ && sizeIndicated > 0 && sizeIndicated <= CO_CONFIG_SDO_CLI_PST)
        {
            SDO_C->state = CO_SDO_ST_DOWNLOAD_INITIATE_REQ;
        }
#endif
                                           ...
                                           uint32_t *timerNext_us)
{
    ...
    if (ret == CO_SDOcli_waitingServerResponse)
    {
        ...
        case CO_SDO_ST_DOWNLOAD_INITIATE_REQ:
        {
            ...
            if ((SDO_C->sizeInd == 0 && count <= 4) || (SDO_C->sizeInd > 0 && SDO_C->sizeInd <= 4))
            {
